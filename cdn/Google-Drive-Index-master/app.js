...
function init() {
	...
	var html = `
	  ...
	  <div class="container">
	  <p class="float-right"> <a href="#">Ir Arriba</a> </p> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://github.com/ParveenBhadooOfficial/Bhadoo-Drive-Index" target="_blank">TheFirstSpeedster</a>, based on Open Source Softwares.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a> y <a href=" ${UI.company_link2}" target="_blank"> ${UI.company_name2}</a>, Todos los derechos reservados.</p>
	  </div>
	  </footer>
`;
$('body').html(html);
...
function nav(path) {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item">
        <a class="nav-link" href="/${cur}:/">Inicio</a>
      </li>`;
	...
	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Carpeta Actual</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="/${cur}:/ ">> Inicio</a>`;
    ...
    <a class="nav-link" href="${UI.contact_link}" target="_blank">Contacto</a>
  </li>`;
...
<input class="form-control mr-sm-2" name="q" type="search" placeholder="Buscar" aria-label="Search" value="${search_text}" required>
<button class="btn ${UI.dark_mode ? 'btn-secondary' : 'btn-outline-success'} my-2 my-sm-0" onclick="if($('#search_bar').hasClass('mdui-textfield-expanded') && $('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit">Buscar</button>
</form>
</div>
</nav>
`;
...
function list(path) {
  ...
  <div class="card">
  <h5 class="card-header" id="folderne"><input type="text" id="folderne" class="form-control" placeholder="Current Path: Homepage" value="" readonly><script>document.getElementById("folderne").innerHTML='Carpeta Actual: '+decodeURI(this.window.location.href.substring(window.location.href.lastIndexOf('/',window.location.href.lastIndexOf('/')-1))).replace('/','').replace('/','');</script>
  </h5>
...
function render_search_result_list() {
  ...
  <div class="container"><br>
  <div class="card">
  <h5 class="card-header">Resultados de la Busqueda</h5>
  <div id="list" class="list-group">
  </div>
  </div>
  <div class="card">
